...
class CesiumPrimitivesProvider_CameraCenter2D {
  ...
  batchRender() {
    ...
    // 视线焦点和锥体
    // let cameraPostionCartesian3 = this.camera.position ;//Cartesian3
    // let cameraDirectionCartesian3 = this.camera.direction  ;//Cartesian3
    let windowCoordinates = new Cesium.Cartesian2(this.viewer.canvas.clientWidth/2,this.viewer.canvas.clientHeight/2)
    var ray =  this.viewer.camera.getPickRay(windowCoordinates);
    let endPostion = this.viewer.scene.globe.pick(ray,this.viewer.scene);
    if(!endPostion){//视线与地表无交点
      return
    }
    let cameraPosition = Cesium.Cartographic.fromCartesian(endPostion)
