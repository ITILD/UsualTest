      ...
      let createScene = function () {
        ...
        for (let index = 0; index < 2; index++) {
          const Points = fullJson[index]
            ...
            let upupwalldown = BABYLON.MeshBuilder.CreateLines(
              'lines',
            ...
            for (let index = 0; index < Points.wallup.length; index++) {
              const up = Points.wallup[index];
              const down = Points.walldown[index];
               let upupwallup = BABYLON.MeshBuilder.CreateLines(
              'lines',
              { points: [
                new BABYLON.Vector3(up[0], up[2], up[1]),
                new BABYLON.Vector3(down[0], down[2], down[1])
              ], updatable: true },
              scene
            )
            }
          Points.top84BLH.forEach((json) => {
            let pointThisArray = posintArrayTrans(json.coordinates)
            let upup = BABYLON.MeshBuilder.CreateLines(
              'lines',
              { points: pointThisArray, updatable: true },
              scene
            )
          })
